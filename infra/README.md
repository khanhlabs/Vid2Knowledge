...
4. Set `alert_notification_emails` to two independently monitored addresses for production. Build `backend/Dockerfile`, push it to the new Artifact Registry repository, pin `backend_image` by digest, then run and review a complete `terraform plan` before the full `terraform apply`. Targeted apply is used only for the one-time bootstrap.
